...
class VisibleNodes {
    ...
    public final Node MARKER_NODE = new Node(-99);
    ...
    int visibleNodes(Node root) {
        ...
        Queue<Node> fifoQueue = new ArrayDeque<>();
        if (root == null) {
            return 0;
        }

        fifoQueue.add(MARKER_NODE);
        fifoQueue.add(root);
        int count = 0;
        while (!fifoQueue.isEmpty()) {
            Node n = fifoQueue.poll();

            if (n == MARKER_NODE && !fifoQueue.isEmpty()) {
                count++;
                fifoQueue.add(MARKER_NODE);
                continue;
            }
            if (n.left != null) {
                fifoQueue.offer(n.left);
            }

            if (n.right != null) {
                fifoQueue.offer(n.right);
            }
        }
        return count;
    }
